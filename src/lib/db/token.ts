...
class Token {
  ...
  static async generate(userId: string, req: Request): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    try {
      await Auth.saveRefreshToken({
        ...
        token: refreshToken,
        deviceId,
        expiresAt: new Date(Date.now() + this.getExpiryInMs(Token.REFRESH_EXPIRY))
      });
    } catch (error) {
  ...
  static async renewTokens(refreshToken: string, req: Request): Promise<{ accessToken: string; refreshToken: string } | null> {
    try {
      const { payload } = await jwtVerify(refreshToken, new TextEncoder().encode(this.REFRESH_SECRET)) as { payload: TokenPayload };
      ...
      const newRefreshToken = await new SignJWT({ userId: payload.userId, deviceId: currentDeviceId })
        .setProtectedHeader({ alg: "HS256", typ: "JWT" })
        .setIssuedAt()
        .setIssuer(this.ISSUER)
        .setExpirationTime(Token.REFRESH_EXPIRY)
  ...
  static async isDeviceValid(userId: string, deviceId: string): Promise<boolean> {
    const devices = await Auth.getUserDevices(userId);
    return devices.some(device => device.deviceId === deviceId);
  }

  // Renueva solo el access token si el refresh token es válido
static async renewAccessToken(refreshToken: string, req: Request): Promise<string | null> {
  try {
      const { payload } = await jwtVerify(refreshToken, new TextEncoder().encode(this.REFRESH_SECRET)) as { payload: TokenPayload };
