		  ...
		  <div class="mb-4 flex justify-end items-start space-x-2">
			<div class="max-w-[70%]">
			  <div class="flex flex-col items-end mb-1">
				...
				<span class="text-xs text-gray-500 dark:text-gray-400">{formatTime(new Date(message.createdAt!))}</span>
			  </div>
			...
			<div class="mr-3 h-10 w-10 flex-shrink-0 overflow-hidden rounded-full">
			  <img src="https://randomuser.me/api/portraits/men/1.jpg" alt={message.id} class="h-full w-full object-cover" />
			</div>
		  </div>
		{:else}
		  <!-- Mensaje Recibido -->
