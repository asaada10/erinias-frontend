...
export const POST: RequestHandler = async ({ request, cookies }) => {
	...
	type User = Omit<table.User, 'passwordHash' | 'resetToken' | 'emailVerified' | 'role' | 'twoFactorEnabled'>;

	const results: User[] = await db.select().from(table.user).where(like(table.user.username, `%${search}%`));
